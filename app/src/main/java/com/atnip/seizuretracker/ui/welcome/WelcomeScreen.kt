...
private enum class WelcomeMode { CHOOSE, CREATE, JOIN }

@Composable
fun WelcomeScreen(session: SessionViewModel) {
    var mode by rememberSaveable { mutableStateOf(WelcomeMode.CHOOSE) }
    ...
    Column(
        ...
        when (mode) {
            ...
            WelcomeMode.CREATE -> CreateForm(session, error) { mode = WelcomeMode.CHOOSE }
            WelcomeMode.JOIN -> JoinForm(session, error) { mode = WelcomeMode.CHOOSE }
        }
...
@Composable
private fun CreateForm(session: SessionViewModel, error: String?, onBack: () -> Unit) {
...
@Composable
private fun JoinForm(session: SessionViewModel, error: String?, onBack: () -> Unit) {
